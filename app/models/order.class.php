...
class Order extends Controller
{
	...
	public function validate($POST)
	{
		...
		foreach ($POST as $key => $value) {
			...
			if($key == "country")
			{
				...
				{
					$this->error[] = "Please enter a valid country";
				}
			}
			...
			if($key == "state")
			{
				...
				{
					$this->error[] = "Please enter a valid state";
				}
			}
			...
			if($key == "address1")
			{
				if(empty($value))
				{
					$this->error[] = "Please enter a valid address 1";
				}
			}
	...
	public function save_order($POST,$ROWS,$user_url,$sessionid)
	{
		...
		if(is_array($ROWS) && count($this->error) ==0)
		{
